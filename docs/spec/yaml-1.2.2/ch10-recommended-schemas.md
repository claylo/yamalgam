...
y[schema.failsafe.processor-should-support]
...
y[schema.failsafe.tag-resolution-bang]
...
y[schema.failsafe.tag-resolution-question]
...
y[schema.json.processor-should-support]
A YAML [processor] should therefore support this [schema], at least as an
option.
...
y[schema.json.int-overflow-handling]
...
y[schema.json.float-key-not-recommended]
...
y[schema.json.tag-resolution-bang]
All [nodes] with the "`!`" non-specific tag are [resolved], by the standard
[convention], to "`tag:yaml.org,2002:seq`", "`tag:yaml.org,2002:map`" or
"`tag:yaml.org,2002:str`", according to their [kind].

y[schema.json.tag-resolution-collections]
...
y[schema.json.tag-resolution-scalars]
...
y[schema.json.unmatched-scalar-error]
...
y[schema.json.tag-resolution-table]
...
y[schema.core.recommended-default]
...
y[schema.core.tag-resolution-bang]
All [nodes] with the "`!`" non-specific tag are [resolved], by the standard
[convention], to "`tag:yaml.org,2002:seq`", "`tag:yaml.org,2002:map`" or
"`tag:yaml.org,2002:str`", according to their [kind].

y[schema.core.tag-resolution-collections]
[Collections] with the "`?`" non-specific tag (that is, [untagged]
[collections]) are [resolved] to "`tag:yaml.org,2002:seq`" or
"`tag:yaml.org,2002:map`" according to their [kind].

y[schema.core.tag-resolution-scalars]
[Scalars] with the "`?`" non-specific tag (that is, [plain scalars]) are
matched with an extended list of regular expressions.

y[schema.core.unmatched-scalar-str]
...
y[schema.core.tag-resolution-table]
| Regular expression                | Resolved to tag
| --                                | --
...
y[schema.other.explicit-tags-allowed]
...
y[schema.other.local-tags-insufficient]
...
y[schema.other.interoperable-global-tags]
...
y[schema.other.recommended-base-core]
